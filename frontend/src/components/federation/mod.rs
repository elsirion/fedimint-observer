...
use leptos::{component, create_resource, IntoView, Show, SignalGet, SignalWith, view};
use leptos_router::{Params, ParamsError, ParamsMap, use_params};
use crate::BASE_URL;
...
pub fn Federation() -> impl IntoView {
    ...
    let config_resource = create_resource(id, |id| async move {
        ...
        let config = fetch_federation_config(id).await.map_err(|e| e.to_string())?;
...
async fn fetch_federation_meta(id: FederationId) -> Result<BTreeMap<String, serde_json::Value>, anyhow::Error> {
