...
fn get_network(cfg: &JsonClientConfig) -> String {
    ...
    cfg.modules.iter().find_map(|(_, m)| {
        if m.kind().as_str() != "wallet" {
            return None
        }

        Some(m.value()["network"].as_str().expect("Network is of type string").to_owned())
    }).expect("Wallet module is expected to be present")
}

fn get_modules(cfg: &JsonClientConfig) -> Vec<String> {
    cfg.modules.iter().map(|(_, m)| m.kind().as_str().to_owned()).collect()
}
...
fn get_confirmations_required(cfg: &JsonClientConfig) -> u64 {
    ...
    cfg.modules.iter().find_map(|(_, m)| {
        ...
        Some(m.value()["finality_delay"].as_u64().expect("finality_delay is of type integer") + 1)
    }).expect("Wallet module is expected to be present")
}